...
func (k *Kernel) handleControl(ctx context.Context, redisID string, env protocolEnvelopeLite, raw []byte) bool {
    ...
    case "ensure_schema_subject":
        ...
        _ = k.rd.C().XAdd(ctx, &data.XAddArgs{Stream: prefixed(k.cfg.Redis.KeyPrefix, k.cfg.Redis.ControlRespStream), MaxLen: k.cfg.Redis.MaxLenApprox, Approx: true, Values: map[string]any{"payload": string(b), "ts": time.Now().UnixNano()}}).Err()
        return true
    default:
        return false
    }
