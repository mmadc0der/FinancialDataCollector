...
-- Add superseded status, enforce single active key per producer, add key rotation tracking

-- Add superseded status to producer_keys.status
ALTER TABLE public.producer_keys 
ADD CONSTRAINT check_status CHECK (status IN ('pending', 'approved', 'revoked', 'superseded'));

-- Add superseded tracking columns
ALTER TABLE public.producer_keys 
ADD COLUMN superseded_at TIMESTAMPTZ,
ADD COLUMN superseded_by TEXT REFERENCES public.producer_keys(fingerprint);

-- Make producer_id NOT NULL (no more orphaned keys)
-- First, clean up any existing orphaned keys
...
-- Add rejected status to producer_registrations.status
...
CREATE UNIQUE INDEX idx_producer_keys_one_approved_per_producer 
