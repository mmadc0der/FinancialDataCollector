...
func TestBatchBackpressure_MultipleFlushes(t *testing.T) {
    ...
    if err := pool.QueryRow(context.Background(), `INSERT INTO public.schemas(schema_id,name,version,body) VALUES (gen_random_uuid(),'bp',1,'{}'::jsonb) RETURNING schema_id`).Scan(&schemaID); err != nil { t.Fatalf("schema: %v", err) }
    if err := pool.QueryRow(context.Background(), `INSERT INTO public.producers(producer_id,name) VALUES (gen_random_uuid(),'bp-prod') RETURNING producer_id`).Scan(&producerID); err != nil { t.Fatalf("producer: %v", err) }
    if err := pool.QueryRow(context.Background(), `INSERT INTO public.subjects(subject_id,subject_key,attrs) VALUES (gen_random_uuid(),'BP-1','{}'::jsonb) RETURNING subject_id`).Scan(&subjectID); err != nil { t.Fatalf("subject: %v", err) }
    ...
    cfg := kernelcfg.Config{
        ...
        Postgres: itutil.NewPostgresConfigNoMigrations(dsn, 5, 200, ""), // small batch size, modest wait
        Redis:    kernelcfg.RedisConfig{Addr: addr, KeyPrefix: "fdc:", Stream: "events", PublishEnabled: false, ConsumerGroup: "kernel", ReadCount: 20},
