...
func TestSubjectRegister_ReplayProtection_ResponseAndTTL(t *testing.T) {
    ...
    if err := pool.QueryRow(context.Background(), `INSERT INTO public.producers(producer_id,name) VALUES (gen_random_uuid(),'subr') RETURNING producer_id`).Scan(&producerID); err != nil { t.Fatalf("producer: %v", err) }
    ...
    // First request
    if err := r.XAdd(context.Background(), &redis.XAddArgs{Stream: stream, Values: map[string]any{"pubkey": pubLine, "payload": string(canon), "nonce": nonce, "sig": sigB64}}).Err(); err != nil { t.Fatalf("xadd1: %v", err) }
    // Wait first response
    ...
    if err := r.XAdd(context.Background(), &redis.XAddArgs{Stream: stream, Values: map[string]any{"pubkey": pubLine, "payload": string(canon), "nonce": nonce, "sig": sigB64}}).Err(); err != nil { t.Fatalf("xadd2: %v", err) }

    // Wait for at least 2 messages and assert last is error=replay
    end2 := time.Now().Add(10 * time.Second)
    for time.Now().Before(end2) {
        if l, _ := r.XLen(context.Background(), respStream).Result(); l >= 2 { break }
        time.Sleep(100 * time.Millisecond)
    }
    last, _ := r.XRevRangeN(context.Background(), respStream, "+", "-", 1).Result()
    if len(last) == 0 { t.Fatalf("no response after replay") }
